...
Page 50656 "Posted Finance UpLoads Header"
{
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            part("50000"; "Finance UpLoads Lines")
            {
            ...
            group(ActionGroup1102755021)
            {
                ...
                action("Process Salaries")
                {
                    ...
                    begin
                        ...
                        if "Scheduled Amount" > "Balancing Account Balance" then
    ...
    begin
        ...
        GenJournalLine."account type"::Vendor, ObjSalaryLines."Account No.", "Posting date", ObjSalaryLines.Amount * -1, 'FOSA', EXTERNAL_DOC_NO, ObjSalaryLines."Credit Narration", '', GenJournalLine."application source"::" ");
        exit(RunningBalance);
    end;
    ...
    begin
        ...
        salarybuffer.SetRange("Salary Header No.", No);
        if salarybuffer.Find('-') then begin
            Window.Open(Format("Account Balancing Type") + ': @1@@@@@@@@@@@@@@@' + 'Record:#2###############');
            TotalCount := salarybuffer.Count;
            repeat
                Percentage := (ROUND(Counter / TotalCount * 10000, 1));
                Counter := Counter + 1;
